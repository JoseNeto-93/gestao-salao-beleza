...
const SidebarItem = ({ icon, label, active, onClick }: any) => (
  <button
    onClick={onClick}
    className={`w-full flex items-center space-x-4 px-6 py-4 rounded-2xl transition-all duration-400 group ${active
...
const MobileNavItem = ({ icon, label, active, onClick }: any) => (
  <button
    onClick={onClick}
    className={`flex flex-col items-center justify-center flex-1 transition-all ${active ? 'text-[#ff007a]' : 'text-white/30'
      }`}
  >
...
export default function App() {
  ...
  const [backendUrl, setBackendUrl] = useState(() => {
    ...
    return (window as any).CONFIG_BACKEND_URL || localStorage.getItem('bellaflow_backend_url') || 'http://localhost:3001';
  });
